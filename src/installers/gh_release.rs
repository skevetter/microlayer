...
pub fn install(
    ...
    download_and_install(&client, &asset.browser_download_url, binary_names, bin_location)?;
...
fn find_suitable_asset<'a>(assets: &'a [Asset], arch: &str, os: &str) -> Option<&'a Asset> {
    ...
    for asset in assets {
        ...
        let has_arch = arch_patterns.iter().any(|p| name_lower.contains(&p.to_lowercase()));
        let has_os = os_patterns.iter().any(|p| name_lower.contains(&p.to_lowercase()));
